Flocoin Core version *0.15.1* is now available from:

  <https://download.flocoin.org/flocoin-0.15.1/>
...
A number of changes to the way Flocoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 blessings per kB (0.00001 FCI). This is to prepare for dropping the minimum transaction fee to 0.1 blessings per kB (0.0001 FCI) in the next release.
